...
func GetCronList(path string) (cron []Command, cronRtime int64, err error) {
	if path == "" {
		path = GetConfPath()
	}
...
func CheckCron(path string, lastTime int64) (stat bool, cron []Command, cronRtime int64, err error) {
	stat = false
	fileStat, err := os.Stat(path + "/Conf/cron.json")
	if err != nil {
		return
